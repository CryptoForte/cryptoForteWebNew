...
const CommunityFeedbackCarousel = () => {
    const [currentSlide, setCurrentSlide] = useState(1); // Start with the center slide
    ...
    return (
      <section className="py-8 bg-primary">
        <div className="w-full max-w-sm lg:max-w-4xl mx-auto flex overflow-hidden ">
          {/* Slides */}          
          <div
          ...
          >
            {slides.map((slide) => (
              <div
                key={slide.id}
                className={`w-full flex-shrink-0 px-4 p-8 bg-white rounded-lg shadow-lg`}
              >
                < SiFsecure className="text-2xl md:text-4xl"/>  
                <p className="text-gray-700 text-justify mt-4">{slide.content}</p>
                <article className="flex items-center gap-3 mt-4">
                  <div className="bg-blue-400 p-2 rounded-md">
                    {slide.image ? slide.image : <FaUser className="text-xl text-white"/>}
                  </div>
                  <div>
